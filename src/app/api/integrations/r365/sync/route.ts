...
} from "@/lib/integrations/r365/sync";
...
const syncSchema = z.object({
  ...
  dataTypes: z.array(z.enum(["labor", "sales", "transactions", "invoices"])).optional(),
});
...
/**
 * POST /api/integrations/r365/sync
 * Trigger R365 data sync
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { integrationId, startDate, endDate, fullSync, dataTypes } = syncSchema.parse(body);
    ...
    if (integration.status !== "CONNECTED") {
      return NextResponse.json(
        { error: "R365 integration is not connected" },
        { status: 400 }
      );
    }
    ...
    if (fullSync) {
      ...
      result = await syncR365Data(integrationId, {
        startDate: startDate ? new Date(startDate) : undefined,
        endDate: endDate ? new Date(endDate) : undefined,
        dataTypes: dataTypes as Array<"labor" | "sales" | "transactions" | "invoices">,
      });
    }
    ...
    console.error("R365 sync error:", error);
...
/**
 * GET /api/integrations/r365/sync?jobId=xxx
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("R365 sync progress error:", error);
